...
# Use the correct CSV files for training and validation
...
# Parameter columns for ONEPILLAR case
...
# ============================================================
# CHECK DATA RANGES (NO FILTERING)
# ============================================================
print("\n=== DATA RANGES ===")
print(f"u: min={df_train['u'].min():.2e}, max={df_train['u'].max():.2e}")
print(f"v: min={df_train['v'].min():.2e}, max={df_train['v'].max():.2e}")
print(f"w: min={df_train['w'].min():.2e}, max={df_train['w'].max():.2e}")
print(f"p: min={df_train['p'].min():.2e}, max={df_train['p'].max():.2e}")
print(f"x: min={df_train['x'].min():.2e}, max={df_train['x'].max():.2e}")
print(f"y: min={df_train['y'].min():.2e}, max={df_train['y'].max():.2e}")
print(f"z: min={df_train['z'].min():.2e}, max={df_train['z'].max():.2e}")

# Check simulations
print(f"\nSimulations in training: {df_train['sim'].unique()}")
print(f"Simulations in validation: {df_val['sim'].unique()}")
...
def normalize_simulation_data(df):
    ...
    for sim in df['sim'].unique():
        ...
        z_min, z_max = df_sim['z'].min(), df_sim['z'].max()
        u_min, u_max = df_sim['u'].min(), df_sim['u'].max()
        v_min, v_max = df_sim['v'].min(), df_sim['v'].max()
        w_min, w_max = df_sim['w'].min(), df_sim['w'].max()
        p_min, p_max = df_sim['p'].min(), df_sim['p'].max()
        
        print(f"  Sim {sim}: u=[{u_min:.2e}, {u_max:.2e}], p=[{p_min:.2e}, {p_max:.2e}]")
        
        # Store scales
        sim_scales[sim] = {
            'x': (x_min, x_max), 'y': (y_min, y_max), 'z': (z_min, z_max),
            'u': (u_min, u_max), 'v': (v_min, v_max), 'w': (w_min, w_max),
            'p': (p_min, p_max)
        }
        ...
        z_norm = (df_sim['z'].values - z_min) / (z_max - z_min + 1e-12)
        u_norm = (df_sim['u'].values - u_min) / (u_max - u_min + 1e-12)
        v_norm = (df_sim['v'].values - v_min) / (v_max - v_min + 1e-12)
        w_norm = (df_sim['w'].values - w_min) / (w_max - w_min + 1e-12)
    ...
    df_norm['z'] = np.concatenate(z_norm_list)
    df_norm['u'] = np.concatenate(u_norm_list)
    df_norm['v'] = np.concatenate(v_norm_list)
    df_norm['w'] = np.concatenate(w_norm_list)
...
z_train_n = df_train_norm["z"].values.astype(np.float32)
u_train_n = df_train_norm["u"].values.astype(np.float32)
v_train_n = df_train_norm["v"].values.astype(np.float32)
w_train_n = df_train_norm["w"].values.astype(np.float32)
...
z_val_n = df_val_norm["z"].values.astype(np.float32)
u_val_n = df_val_norm["u"].values.astype(np.float32)
v_val_n = df_val_norm["v"].values.astype(np.float32)
w_val_n = df_val_norm["w"].values.astype(np.float32)
...
z_train = torch.tensor(z_train_n, dtype=torch.float32, device=device).view(-1, 1)
...
w_train_n_t = torch.tensor(w_train_n, dtype=torch.float32, device=device).view(-1, 1)
...
z_val = torch.tensor(z_val_n, dtype=torch.float32, device=device).view(-1, 1)
...
w_val_n_t = torch.tensor(w_val_n, dtype=torch.float32, device=device).view(-1, 1)
...
# ============================================================
# 4. PARAMETRIC PINN ARCHITECTURE (3D)
# ============================================================

class PINN(nn.Module):
    def __init__(self, in_dim=3 + N_PARAMS, out_dim=4, width=64, depth=6):
        super().__init__()
        layers = []
...
for epoch in range(1, N_EPOCHS + 1):
    ...
    model.eval()
    with torch.no_grad():
        u_val_pred_n, v_val_pred_n, w_val_pred_n, p_val_pred_n = model(x_val, y_val, z_val, params_val_n_tensor)
        val_loss_data = torch.mean(
            (u_val_pred_n - u_val_n_t) ** 2 +
            (v_val_pred_n - v_val_n_t) ** 2 +
            (w_val_pred_n - w_val_n_t) ** 2 +
...
torch.save(checkpoint, "models/pinn_onepillar.pt")
print("\nModel saved to 'models/pinn_onepillar.pt'")
